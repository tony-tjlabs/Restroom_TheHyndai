...
- 주중: 직장인 행동이 시간대에 묶여 있어 특정 시간에 패턴이 뚜렷함
  · 오전 개점~12시: 쇼핑 목적 방문객 입장 (1F 통행 상승)
  · 12시경: 오전 입장객은 식당가(B1/5F/6F)로 이동 + 직장인은 아직 점심 중 → 1F 통행 일시 감소 (딥 현상)
  · 13시~: 점심 마친 직장인 유입 + 오후 방문객 → 재상승
  · 17~18시: 퇴근 후 유입 피크
- 주말: 가족/커플 등 자유 시간대 방문 → 특정 시간에 묶이지 않아 부드러운 곡선
- 주중/주말 유동인구 차이가 크며 (주말이 1.5~2배), 이용 패턴도 근본적으로 다름
...
def generate_insights(metrics: dict) -> dict[str, str]:
    ...
    if other_days:
        ...
        for od in other_days:
            ...
            od_dow_kr = owi.get("day_kr", "?")
            try:
                od_dt = _dt.strptime(d_str, "%Y-%m-%d")
                od_dow = od_dt.weekday()
            except Exception:
                od_dow = -1

            od_info = {**od, "day_kr": od_dow_kr, "dow": od_dow}

            if od_dow == today_dow:
                same_weekday.append(od_info)
            if today_dt and od_dt and (today_dt - od_dt).days == 1:
                yesterday = od_info
            if od_dow < 5:
                weekday_group.append(od_info)
            else:
                weekend_group.append(od_info)
        ...
        if same_weekday:
            lines = [_fmt_detail(od) for od in same_weekday]
            blocks.append(f"[같은 요일({_day_kr[today_dow]}요일) 비교]\n" + "\n".join(lines))

        def _avg_group(group, label):
            if not group:
                return None
            n = len(group)
            avg_ft = sum(o.get("foot_traffic", 0) for o in group) / n
            avg_users = sum(o.get("total_users", 0) for o in group) / n
            avg_rate = sum(o.get("usage_rate", 0) for o in group) / n
            avg_male = sum(o.get("male_avg_min", 0) for o in group) / n
            avg_female = sum(o.get("female_avg_min", 0) for o in group) / n
            return (
                f"  {label} 평균 ({n}일): 유동 {avg_ft:,.0f} / 이용 {avg_users:,.0f}명, "
                f"이용률 {avg_rate:.1f}%, 남 체류 {avg_male:.1f}분 / 여 체류 {avg_female:.1f}분"
            )

        is_weekend = today_dow >= 5
        avg_lines = []
        wd = _avg_group(weekday_group, "주중(월~금)")
        we = _avg_group(weekend_group, "주말(토~일)")
        if wd:
            avg_lines.append(wd)
        if we:
            avg_lines.append(we)
        if avg_lines:
            today_type = "주말" if is_weekend else "주중"
            blocks.append(f"[주중/주말 평균 비교 — 오늘은 {today_type}]\n" + "\n".join(avg_lines))

        all_lines = [_fmt_detail(od) for od in other_days]
        blocks.append("[전체 기록 날짜]\n" + "\n".join(all_lines))
...
{other_block}
아래 5개 섹션에 대해 각각 2~3문장의 인사이트를 작성하세요.
비교 데이터가 있으면 반드시 활용하여 "전일 대비", "같은 요일 대비", "주중/주말 평균 대비" 등 상대적 분석을 포함하세요.
단순 수치 나열이 아닌, 변화의 원인(요일/날씨/시간대)을 추정하고 시설 관리 액션을 제안하세요.
반드시 아래 형식을 지켜주세요:
